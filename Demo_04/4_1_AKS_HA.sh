...
az disk list --resource-group MC_Lightup-2020_endurance_westus --query '[].{Name:name, Size:diskSizeGb, DiskState:diskState}' -o table

# Filtering by individual disk
az_disk=`kubectl describe pvc pvc-data-plex | grep "Volume:" | awk '{print $2}'`
az disk list --resource-group MC_Lightup-2020_endurance_westus --query "[?name=='kubernetes-dynamic-$az_disk'].{Name:name, Size:diskSizeGb, DiskState:diskState}" --output table
